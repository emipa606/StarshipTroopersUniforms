...
namespace GraphicApparelDetour
{
	...
	internal static class InjectorThingy
	{
		...
		{
			MethodInfo method = typeof(RimWorld.ApparelGraphicRecordGetter).GetMethod("TryGetGraphicApparel", BindingFlags.Static | BindingFlags.Public);
			...
			var flag = !Detours.TryDetourFromTo(method, method2);
			if (flag)
			{
        ...
        private static string AssemblyName => InjectorThingy.Assembly.FullName.Split(new char[]
                {
                    ','
                }).First<string>();
    }
